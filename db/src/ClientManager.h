...
class CClientManager : public CNetBase, public singleton<CClientManager>
{
	...
	public:
		bool		InitializeMaintenance(bool updateFromGameMaster = false);
		void		RecvMaintenanceData(const char* data);
		void		UpdateMaintenance();
		void		SendMaintenanceData(CPeer* pkPeer = NULL, bool updateFromGameMaster = false);

	protected:
		std::vector<TMainteance> m_MaintenanceManager;
#endif
...
public:
	...
	void	AddTimePremium(DWORD p);
	void	MoveItem(DWORD ownerID, WORD slotPos, WORD targetPos);
	void	RemoveAllItem(DWORD ownerID);
	void	ChangePrice(DWORD ownerID, WORD slotPos, long long itemPrice);
