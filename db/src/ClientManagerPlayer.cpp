...
bool CreateItemTableFromRes(MYSQL_RES * res, std::vector<TPlayerItem> * pVec, DWORD dwPID)
{
	...
	for (int i = 0; i < rows; ++i)
	{
		...
		str_to_number(item.alSockets[7], row[cur++]);
...
size_t CreatePlayerSaveQuery(char * pszQuery, size_t querySize, TPlayerTable * pkTab)
{
	...
	queryLen = snprintf(pszQuery, querySize,
		...
		pkTab->shopFlag,
#endif
		pkTab->iNormalInventory,
		pkTab->iSkillbookInventory,
		pkTab->iMaterialInventory,
		pkTab->iStoneInventory,
		pkTab->iChestInventory,
		pkTab->iEnchantInventory,
		pkTab->iCostumeInventory);
	...
	CDBManager::instance().EscapeString(text, pkTab->statsData, sizeof(pkTab->statsData));
	queryLen += snprintf(pszQuery + queryLen, querySize - queryLen, "stats = '%s' ", text);
#endif
...
void CClientManager::PutPlayerCache(TPlayerTable * pNew)
{
	...
	if (!c)
	{
		...
		m_map_playerCache.emplace(TPlayerTableCacheMap::value_type(pNew->id, c));
	}
...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		sys_log(0, "[PLAYER_LOAD] ID %s pid %d gold %lld "
#if defined(__GEM_SYSTEM__)
			"gem %d "
#endif
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				", shop_flag"
#endif
				", inventory, skill_inventory, material_inventory, stone_inventory, chest_inventory, enchant_inventory, costume_inventory"
...
#ifdef NEW_STATISTICS
	if (row[col])
		thecore_memcpy(pkTab->statsData, row[col], sizeof(pkTab->statsData));
	else
		memset(&pkTab->statsData, 0, sizeof(pkTab->statsData));
	col++;
#endif
...
#endif
	str_to_number(pkTab->iNormalInventory, row[col++]);
	str_to_number(pkTab->iSkillbookInventory, row[col++]);
	str_to_number(pkTab->iMaterialInventory, row[col++]);
	str_to_number(pkTab->iStoneInventory, row[col++]);
	str_to_number(pkTab->iChestInventory, row[col++]);
	str_to_number(pkTab->iEnchantInventory, row[col++]);
	str_to_number(pkTab->iCostumeInventory, row[col++]);
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
	...
	queryLen = snprintf(queryStr, sizeof(queryStr),
			...
			"inventory, skill_inventory, material_inventory, stone_inventory, chest_inventory, enchant_inventory, costume_inventory, "
			"playtime, "
			"skill_level, quickslot, battlepass, stats) "
			...
			, packet->player_table.iNormalInventory
			, packet->player_table.iSkillbookInventory
			, packet->player_table.iMaterialInventory
			, packet->player_table.iStoneInventory
			, packet->player_table.iChestInventory
			, packet->player_table.iEnchantInventory
			, packet->player_table.iCostumeInventory
			);
	...
	CDBManager::instance().EscapeString(text, packet->player_table.battlePassData, sizeof(packet->player_table.battlePassData));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, "'%s', ", text);
	if (g_test_server)
		sys_log(0, "Create_Player queryLen[%d] TEXT[%s]", queryLen, text);

	CDBManager::instance().EscapeString(text, packet->player_table.statsData, sizeof(packet->player_table.statsData));
	queryLen += snprintf(queryStr + queryLen, sizeof(queryStr) - queryLen, "'%s')", text);
	if (g_test_server)
		sys_log(0, "Create_Player queryLen[%d] TEXT[%s]", queryLen, text);
