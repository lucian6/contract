...
void ItemAwardManager::RequestLoad()
{
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT id,login,vnum,count,socket0,socket1,socket2,socket3,socket4,socket5,socket6,socket7,"
		#ifdef ENABLE_EXTEND_ITEM_AWARD
...
void ItemAwardManager::Load(SQLMsg * pMsg)
{
	...
	for (uint i = 0; i < pMsg->Get()->uiNumRows; ++i)
	{
		...
		str_to_number(kData->dwSocket7, row[col++]);
		#ifdef ENABLE_EXTEND_ITEM_AWARD
