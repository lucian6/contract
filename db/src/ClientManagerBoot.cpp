...
bool CClientManager::InitializeTables()
{
	...
	if (!InitializeMaintenance())
	{
		sys_err("InitializeMaintenance FAILED");
		return false;
	}
#endif
	...
	for (itertype(m_vec_mobTable) it = m_vec_mobTable.begin(); it != m_vec_mobTable.end(); it++)
	{
		...
		if (g_stLocaleNameColumn == "name")
		{
			snprintf(query, sizeof(query),
				...
				"resist_fire, resist_elect, resist_magic, resist_wind, resist_poison, resist_critical, "
...
{	char query[2048];
	...
	snprintf(query, sizeof(query),
		...
		" att_elec, att_fire, att_ice, att_wind, att_earth, att_dark,"
		" resist_fire, resist_elect, resist_magic, resist_wind, resist_poison, resist_critical, dam_multiply, summon, drain_sp,"
		" skill_vnum0, skill_level0, skill_vnum1, skill_level1, skill_vnum2, skill_level2, skill_vnum3, skill_level3,"
		" skill_vnum4, skill_level4, sp_berserk, sp_stoneskin, sp_godspeed, sp_deathblow, sp_revive"
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		VERIFY_IFIELD(MProto::resist_critical,	mob_table->cResists[MOB_RESIST_CRITICAL]);

		VERIFY_IFIELD(MProto::att_elec,			mob_table->cElements[MOB_ELEMENT_ELECT]);
		VERIFY_IFIELD(MProto::att_fire,			mob_table->cElements[MOB_ELEMENT_FIRE]);
		VERIFY_IFIELD(MProto::att_ice,			mob_table->cElements[MOB_ELEMENT_ICE]);
		VERIFY_IFIELD(MProto::att_wind,			mob_table->cElements[MOB_ELEMENT_WIND]);
		VERIFY_IFIELD(MProto::att_earth,		mob_table->cElements[MOB_ELEMENT_EARTH]);
		VERIFY_IFIELD(MProto::att_dark,			mob_table->cElements[MOB_ELEMENT_DARK]);
		// ## OTHERS #2
...
// 	if (item_table->dwShopBuyPrice > item_table->dwGold)
// 	{
...
// 			item_table->dwGold
// 		);
// 		item_table->dwShopBuyPrice = item_table->dwGold;
// 	}
// #endif
