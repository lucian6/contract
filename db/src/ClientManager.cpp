...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (pi->account_index == 0)
	{
		...
		char szQuery[1024]; // crash query prea lung
		snprintf(szQuery, sizeof(szQuery),
				"SELECT id, `window`+0, pos, count, vnum, socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7, "
			...
			if (!bEscape)
			{
				...
				while (it != pSet->end())
				{
...

					{
						...
						auto iLen = snprintf(szColumns, sizeof(szColumns), "id, owner_id, `window`, pos, count, vnum");
						auto iValueLen = snprintf(szValues, sizeof(szValues), "%u, %u, '%s', %d, %u, %d", GainItemID(), pi->account_id, (pi->ip[0] == 0) ? "SAFEBOX" : "MALL", iPos, pItemAward->dwCount, pItemAward->dwVnum);

						iLen += snprintf(szColumns + iLen, sizeof(szColumns) - iLen, ", socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7");
						iValueLen += snprintf(szValues + iValueLen, sizeof(szValues) - iValueLen, ", %u, %u, %u, %u, %u, %u, %u, %u", pItemAward->dwSocket0, pItemAward->dwSocket1, pItemAward->dwSocket2, pItemAward->dwSocket3, pItemAward->dwSocket4, pItemAward->dwSocket5, pItemAward->dwSocket6, pItemAward->dwSocket7);
...
#else
						snprintf(szQuery, sizeof(szQuery),
								"INSERT INTO item%s (id, owner_id, `window`, pos, vnum, count, socket0, socket1, socket2, socket3, socket4, socket5, socket6, socket7) "
								"VALUES(%u, %u, '%s', %d, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u)",
								GetTablePostfix(),
								GainItemID(),
								pi->account_id,
					...
					item.alSockets[6] = dwSocket6;
					item.alSockets[7] = dwSocket7;
					#ifdef ENABLE_EXTEND_ITEM_AWARD
...
#ifdef __MAINTENANCE__
		r.isTester = p->isTester;
#endif

		sys_log(0, "AUTH_LOGIN id(%u) login(%s) language(%d) social_id(%s) login_key(%u), client_key(%u %u %u %u)",
				p->dwID, p->szLogin, p->bLanguage, p->szSocialID, p->dwLoginKey,
...
#ifdef __MAINTENANCE__
		pkLD->GetAccountRef().isTester = data->isTester;
#endif
	}
}

#ifdef __MAINTENANCE__
bool CClientManager::InitializeMaintenance(bool updateFromGameMaster)
{
	m_MaintenanceManager.clear();
	char szQuery[124];
	snprintf(szQuery, sizeof(szQuery), "SELECT id, mode+0, UNIX_TIMESTAMP(start), UNIX_TIMESTAMP(end), reason FROM player.maintenance");
	...
	if (pMsg->Get()->uiNumRows != 0)
	{
		...
		for (int n = 0; (row = mysql_fetch_row(pMsg->Get()->pSQLResult)) != NULL; ++n)
		{
			...
			str_to_number(p.mode, row[col++]);
			str_to_number(p.iStartTime, row[col++]);
			str_to_number(p.iEndTime, row[col++]);
			strlcpy(p.reason, row[col++], sizeof(p.reason));
			p.status = (p.iStartTime <= cur_Time && cur_Time <= p.iEndTime);
			if (cur_Time > p.iEndTime)
				continue;
			m_MaintenanceManager.emplace_back(p);
		}
	}
	sys_log(0, "%s maintenance table.", updateFromGameMaster ? "Reload" : "Init");
	SendMaintenanceData(NULL, updateFromGameMaster);
	return true;
}
void CClientManager::SendMaintenanceData(CPeer* pkPeer, bool updateFromGameMaster)
{
	const BYTE subIndex = MAINTENANCE_DG_LOAD;
	const BYTE dataCount = m_MaintenanceManager.size();
	TEMP_BUFFER buf;
	buf.write(&subIndex, sizeof(BYTE));
	buf.write(&dataCount, sizeof(BYTE));
	buf.write(&updateFromGameMaster, sizeof(bool));
	if (dataCount)
		buf.write(m_MaintenanceManager.data(), dataCount * sizeof(TMainteance));
	if (pkPeer != NULL)
	{
		pkPeer->EncodeHeader(HEADER_DG_MAINTENANCE, 0, buf.size());
		pkPeer->Encode(buf.read_peek(), buf.size());
	}
	else
		ForwardPacket(HEADER_DG_MAINTENANCE, buf.read_peek(), buf.size());

	if (m_pkAuthPeer)
	{
		m_pkAuthPeer->EncodeHeader(HEADER_DG_MAINTENANCE, 0, buf.size());
		m_pkAuthPeer->Encode(buf.read_peek(), buf.size());
	}
}
void CClientManager::UpdateMaintenance()
{
	const time_t now = time(0);
	for (WORD j = 0; j < m_MaintenanceManager.size(); ++j)
	{
		TMainteance& pData = m_MaintenanceManager[j];
		bool sendStatusPacket = false;
		if ((pData.iStartTime - now) == 0 && pData.status == false)
		{
			pData.status = true;
			sendStatusPacket = true;
		}
		if ((pData.iEndTime - now) == 0 && pData.status == true)
		{
			pData.status = false;
			sendStatusPacket = true;
		}
		if (sendStatusPacket)
		{
			const BYTE subIndex = MAINTENANCE_DG_UPDATE;
			TEMP_BUFFER buf;
			buf.write(&subIndex, sizeof(BYTE));
			buf.write(&pData.id, sizeof(WORD));
			buf.write(&pData.status, sizeof(bool));
			ForwardPacket(HEADER_DG_MAINTENANCE, buf.read_peek(), buf.size());
...
void CClientManager::RecvMaintenanceData(const char* data)
{
	const BYTE subIndex = *(BYTE*)data;
	data += sizeof(BYTE);
	if (subIndex == MAINTENANCE_GD_RELOAD)
		InitializeMaintenance(true);
}
#endif
...
void CClientManager::OfflineMessageGarbage()
{
	...
	static const/*expr*/ int iHour = 12;
