...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (CheckDragonSoulGrade(pItem->GetVnum()))
	{
...

	{
		...
		{
			if (CheckDragonSoulGrade(pItem->GetVnum()))
				ch->PointChange(POINT_GOLD, -100000000, true);
...

	{
		...
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("용혼석 추출에 실패하여 %s를 얻었습니다."), pByProduct->GetLocaleName(ch->GetLanguage()));
				else
...
bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (ch && !PulseManager::Instance().IncreaseClock(ch->GetPlayerID(), ePulse::RefineStrength, std::chrono::milliseconds(500)))
		return false;
#endif
...
bool DSManager::DoChangeAttribute(LPCHARACTER lpCh, TItemPos(&arItemPos)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	for (auto lpItem : setItems)
	{
		...
		{
			lpCh->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("등급 개량에 필요한 재료가 아닙니다."));
