...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	SetStatistics(STAT_TYPE_SKILLBOOKS, 1);
...
void CHARACTER::SkillLevelUp(DWORD dwVnum, BYTE bMethod)
{
	...
	if (pkSk->dwVnum < 111 && m_pSkillLevels[pkSk->dwVnum].bLevel == 40)
		CHARACTER_MANAGER::Instance().SetRewardData(REWARD_LEGENDARY_SKILL, GetName(), true);

	if (pkSk->dwVnum == 121 && m_pSkillLevels[pkSk->dwVnum].bLevel == 40)
		CHARACTER_MANAGER::Instance().SetRewardData(REWARD_LEADERSHIP, GetName(), true);
	...
	if (GetJob() == JOB_WARRIOR && GetSkillGroup() == 1)
	{
		...
		{
			CHARACTER_MANAGER::Instance().SetRewardData(REWARD_LEGENDARY_SKILL_SET, GetName(), true);
		}
	...
	if (GetSkillLevel(164) == 40 && GetSkillLevel(165) == 40 && GetSkillLevel(166) == 40 && GetSkillLevel(167) == 40)
		CHARACTER_MANAGER::Instance().SetRewardData(REWARD_BUFFI,GetName(), true);
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		{
			m_pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("효과가 없습니다. 스킬 공식을 확인하세요"));
		}
		////////////////////////////////////////////////////////////////////////////////
		iAmount = -iAmount;
		...
		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun())
		{
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_REMOVE_GOOD_AFFECT))
			{
...
#endif
				{
					...
					if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_SELFONLY) || m_pkSk->dwAffectFlag == AFF_GICHEON || m_pkSk->dwAffectFlag == AFF_HOSIN || m_pkSk->dwAffectFlag == AFF_BOHO || m_pkSk->dwAffectFlag == AFF_GICHEON || m_pkSk->dwAffectFlag == AFF_KWAESOK || m_pkSk->dwAffectFlag == AFF_JEUNGRYEOK || m_pkSk->dwAffectFlag == AFF_BUFFI_HOSIN || m_pkSk->dwAffectFlag == AFF_BUFFI_GICHEON || m_pkSk->dwAffectFlag == AFF_BUFFI_KWAESOK || m_pkSk->dwAffectFlag == AFF_BUFFI_JEUNGRYEOK)
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_CRUSH | SKILL_FLAG_CRUSH_LONG) &&
				!IS_SET(pkChrVictim->GetAIFlag(), AIFLAG_NOMOVE) && m_pkChr->GetRaceNum() != 6192)
			{
...
int CHARACTER::ComputeSkillAtPosition(DWORD dwVnum, const PIXEL_POSITION& posTarget, BYTE bSkillLevel)
{
	...
	if (IS_SET(pkSk->dwFlag, SKILL_FLAG_ATTACK | SKILL_FLAG_USE_MELEE_DAMAGE | SKILL_FLAG_USE_MAGIC_DAMAGE))
	{
		...
		else
		{
			...
			if (iDur > 0)
			{
				if (IS_SET(pkSk->dwFlag, SKILL_FLAG_SELFONLY) || pkSk->dwAffectFlag == AFF_GICHEON || pkSk->dwAffectFlag == AFF_HOSIN || pkSk->dwAffectFlag == AFF_BOHO || pkSk->dwAffectFlag == AFF_GICHEON || pkSk->dwAffectFlag == AFF_KWAESOK || pkSk->dwAffectFlag == AFF_JEUNGRYEOK || pkSk->dwAffectFlag == AFF_BUFFI_HOSIN || pkSk->dwAffectFlag == AFF_BUFFI_GICHEON || pkSk->dwAffectFlag == AFF_BUFFI_KWAESOK || pkSk->dwAffectFlag == AFF_BUFFI_JEUNGRYEOK)
...
int CHARACTER::ComputeSkill(DWORD dwVnum, LPCHARACTER pkVictim, BYTE bSkillLevel)
{
	...
	if (test_server && iAmount == 0 && pkSk->bPointOn != POINT_NONE)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("효과가 없습니다. 스킬 공식을 확인하세요"));
	}
	// END_OF_ADD_GRANDMASTER_SKILL
