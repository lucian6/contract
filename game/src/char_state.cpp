					...
					if (pkAff)
					{
								...
								m_pkChr->GetPoint(POINT_STAT) != pkAff->lApplyValue)
						{
							...
							//if (pMap->IsFlagOnBase(idx))
							{
								...
								pkChr->SetStatistics(STAT_TYPE_GUILD_FLAGS, 1);

								char buf[256];
								snprintf(buf, sizeof(buf), LC_STRING("%s 길드가 %s 길드의 깃발을 빼앗았습니다!", pkChr->GetLanguage()), pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName());
								pMap->Notice(buf);
							}
						}
					}
...
bool __CHARACTER_GotoNearTarget(LPCHARACTER self, LPCHARACTER victim)
{
	...
	switch (self->GetMobBattleType())
	{
		...
		case BATTLE_TYPE_MAGIC:
			...
			if (self->Follow(victim, self->GetMobAttackRange(victim->GetRaceNum()) * 8 / 10))
				return true;
			break;

		default:

			if (self->Follow(victim, self->GetMobAttackRange(victim->GetRaceNum()) * 9 / 10))
				return true;
	}
...
void CHARACTER::StateBattle()
{
	...
	if (fDist >= (GetMobAttackRange(victim->GetRaceNum()) * 1.15) && !victim->IsInSafezone())
	{
		...
		if (!victim || (victim && !victim->IsInSafezone()) || (pkChrProtege && !pkChrProtege->IsInSafezone()))
		{
			for (unsigned int iSkillIdx = 0; iSkillIdx < MOB_SKILL_MAX_NUM; ++iSkillIdx)
			{
				if (CanUseMobSkill(iSkillIdx))
				{
					...
					if (GetRaceNum() == 6193)
					{
						SetRotationToXY(320193, 1523422);
					}
...
void CHARACTER::StateFlag()
{
	...
	snprintf(buf, sizeof(buf), LC_STRING("%s 길드의 깃발을 %s 님이 획득하였습니다.", GetLanguage()), pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName());
	pMap->Notice(buf);
}
...
void CHARACTER::StateHorse()
{
	...
	int		MIN_APPROACH = 200;
	int		MAX_APPROACH = 300;
	float	RESPAWN_DISTANCE = 1700.0f;
...
void CHARACTER::StatePet()
{
	...
	float	START_FOLLOW_DISTANCE = 200.0f;
	float	START_RUN_DISTANCE = 500.0f;
	float	RESPAWN_DISTANCE = 1700.0f;
	int		APPROACH = 150;
...
void CHARACTER::StateBuffi()
{
	float	START_FOLLOW_DISTANCE = 200.0f;
	float	START_RUN_DISTANCE = 500.0f;
	float	RESPAWN_DISTANCE = 1500.0f;
	int		APPROACH = 150;
