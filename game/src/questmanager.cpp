...
#define ENABLE_PARTYKILL
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		else if (name == "valentine_stone_event")
		{
			if (prev_value != value)
			{
				...
				for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
				{
					...
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "valentine_stone_event %d", value);
				}
			}
		}
		else if (name == "icecream_stone_event")
		{
			if (prev_value != value)
			{
				const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
				...
				for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
				{
					...
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "icecream_stone_event %d", value);
				}
			}
		}
		else if (name == "ramadan_stone_event")
		{
			if (prev_value != value)
			{
				const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
				...
				for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
				{
					...
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "ramadan_stone_event %d", value);
				}
			}
		}
		else if (name == "yutnori_stone_event")
		{
			if (prev_value != value)
			{
				const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
				...
				for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
				{
					...
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "yutnori_stone_event %d", value);
				}
			}
		}
		else if (name == "football_stone_event")
		{
			if (prev_value != value)
			{
				const DESC_MANAGER::DESC_SET& c_ref_set = DESC_MANAGER::instance().GetClientSet();
				...
				for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
				{
					...
					if (ch)
						ch->ChatPacket(CHAT_TYPE_COMMAND, "football_stone_event %d", value);
				}
			}
		}
#endif
		...
		{
			g_GoldDropTimeLimitValue = value * 100;
		}
#ifdef ENABLE_NEWSTUFF
		else if (name == "item_drop_limit_time")
		{
			g_ItemDropTimeLimitValue = value * 100;
		}
		else if (name == "box_use_limit_time")
		{
			g_BoxUseTimeLimitValue = value * 100;
		}
		else if (name == "buysell_limit_time")
		{
			g_BuySellTimeLimitValue = value * 100;
		}
...
#if defined(__BL_EVENT_STONE_SHAPE_CHANGE__)
		if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("valentine_stone_event")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "valentine_stone_event %d", iEventFlagValue);
		}
		else if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("icecream_stone_event")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "icecream_stone_event %d", iEventFlagValue);
		}
		else if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("ramadan_stone_event")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "ramadan_stone_event %d", iEventFlagValue);
		}
		else if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("yutnori_stone_event")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "yutnori_stone_event %d", iEventFlagValue);
		}
		else if ((iEventFlagValue = quest::CQuestManager::instance().GetEventFlag("football_stone_event")))
		{
			ch->ChatPacket(CHAT_TYPE_COMMAND, "football_stone_event %d", iEventFlagValue);
		}
#endif
