...
void CGuild::LocaleChat(const char* c_pszText, ...)
{
	for (TGuildMemberOnlineContainer::iterator it = m_memberOnline.begin(); it != m_memberOnline.end(); ++it)
	{
		const LPDESC c_lpDesc = (*it)->GetDesc();
		if (c_lpDesc)
		{
			va_list pszArgs;
			va_start(pszArgs, c_pszText);
			{
				// MULTI_LOCALE_SUPPORT
				const char* c_pszLocaleText = LC_STRING(c_pszText, c_lpDesc->GetLanguage());
				// END_OF_MULTI_LOCALE_SUPPORT

				char szBuf[CHAT_MAX_LEN + 1] = {};
				int iLen = vsnprintf(szBuf, sizeof(szBuf), c_pszLocaleText, pszArgs);

				c_lpDesc->ChatPacket(CHAT_TYPE_GUILD, "%s", szBuf);
			}
			va_end(pszArgs);
		}
	}
...
int CGuild::GetMaxMemberCount()
{
	...
	return 200 + 2 * (m_data.level-1) + m_iMemberCountBonus;
}
