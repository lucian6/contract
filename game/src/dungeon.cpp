...
void CDungeon::DecMember(LPCHARACTER ch)
{
	...
	if (m_set_pkCharacter.empty() && GetFlag("dungeon_block") == 0)
	{
...
int CDungeon::CountMember() const
{
    return m_set_pkCharacter.size();
}
...
void CDungeon::UniqueSetHP(const std::string& key, int iHP)
{
	...
	it->second->SetHP(iHP);
	it->second->BroadcastTargetPacket();
}
...
void CDungeon::UniqueSetAffect(const std::string& key, int type, int flag, int duration)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
	...
	it->second->AddAffect(type, POINT_NONE, 0, flag, duration, 0, 0, true);
}

void CDungeon::UniqueRemoveAffect(const std::string& key, DWORD type)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
...
bool CDungeon::GetUniquePresence(const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
		return false;

	LPCHARACTER unique = it->second;
	if (unique && unique->GetVID() > 0)
		return true;

	return false;
}

DWORD CDungeon::GetUniqueMinHP(const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
...
DWORD CDungeon::GetUniqueMaxHP(const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
...
void CDungeon::AttackUniqueCharacter(long lMapIndex, const std::string& key)
{
	TUniqueMobMap::iterator it = m_map_UniqueMob.find(key);
	if (it == m_map_UniqueMob.end())
	{
	...
	auto FFindAllianceCharacter = [&chAlly](LPENTITY lpEntity)
	{
		...
		{
			if (chAlly && !chAlly->IsDead())
				c_lpChar->SetVictim(chAlly);
		}
	};

	pkSectreeMap->for_each(FFindAllianceCharacter);	
}
	...
	struct FKillSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsHorse() && !ch->IsHydraContent())
					ch->DeadNoReward(); // @fixme188 from Dead()
			}
		}
	};
	...
	struct FPurgeSectree
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsBuffi() && !ch->IsHorse()) {
					M2_DESTROY_CHARACTER(ch);
				}
			}
	...
	while (it != c_ref_set.end())
	{
		...
		if (d->GetCharacter())
		{
			...
			while ((pos = strBuff.find(strDelim)) != std::string::npos)
			{
				...
				d->GetCharacter()->ChatPacket(CHAT_TYPE_MISSION, "%s", c_pszToken);

				c_pszLast = strBuff.erase(0, pos + strDelim.length()).c_str();
			}
			d->GetCharacter()->ChatPacket(CHAT_TYPE_MISSION, "%s", c_pszLast);
		}
	}
}
