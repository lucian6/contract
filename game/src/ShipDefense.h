...
namespace ShipDefense
{
	...
	enum EWaveDuration : DWORD
	{
		...
		COOLDOWN = 10800, // 30 minutes
		COOLDOWN_HALF = 10800 / 2,
	};
...
class CShipDefense : public singleton<CShipDefense>
{
public:
...
public:
	...
	void SetFastestTime(int time);
	void IncreaseCompletedDungeon();
	void UpdateInfoTimer();

public:
	time_t GetStartTime() const { return m_lStartTime; }
	...
	void SetTest(bool state) { test_dungeon = state; }
	bool GetTest() { return test_dungeon; }
...
public:
	...
	void SetFastest(const LPCHARACTER c_lpChar, int time);
	void IncreaseCompleted(const LPCHARACTER c_lpChar);
	void UpdateInfo(const LPCHARACTER c_lpChar);

public:
	// Leader Actions
	bool Create(const LPCHARACTER c_lpChar);
	bool Start(const LPCHARACTER c_lpChar);
	bool StartTest(const LPCHARACTER c_lpChar);
	...
	bool SetTest(const LPCHARACTER c_lpChar, bool state);
	bool GetTest(const LPCHARACTER c_lpChar);
