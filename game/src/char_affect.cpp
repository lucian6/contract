...
bool IsRealTimeAffect(DWORD affectType)
{
	switch(affectType)
	{
		...
		case AFFECT_SINGLE_BONUS_URIEL:
		case AFFECT_SINGLE_BONUS_URIEL_DELAY:
		case AFFECT_NAME_MOUNT:
		case AFFECT_NAME_PET:
		case AFFECT_DECORATION:
		case AFFECT_THIEF_GLOVES_BOSSES:
		case AFFECT_THIEF_GLOVES_STONES:
		case AFFECT_AUTO_METIN_FARM:
			return true;
	}
	return false;
}
...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
...
#ifdef __RENEWAL_CRYSTAL__
			if (pkAff->dwType == AFFECT_CRYSTAL)
			{
				++it;
				continue;
			}
			...
			if (pkAff->dwType >= AFFECT_BLEND_POTION_1 && pkAff->dwType <= AFFECT_BLEND_MONSTERS)
			{
				++it;
				continue;
			}
...
#ifdef RENEWAL_PICKUP_AFFECT
	if (IsRealTimeAffect(dwType))
	{
		long lRealTimeDur = time(0) + lDuration;
		pkAff->lDuration = (lDuration >= INFINITE_AFFECT_DURATION) ? lDuration : lRealTimeDur;
	}
	else 
	{
...
void CHARACTER::ComputeAffect(CAffect * pkAff, bool bAdd)
{
	...
	if (bAdd)
		PointChange(pkAff->bApplyOn, static_cast<int64_t>(pkAff->lApplyValue));
	else
		PointChange(pkAff->bApplyOn, -static_cast<int64_t>(pkAff->lApplyValue));
...
#ifdef __RENEWAL_CRYSTAL__
void CHARACTER::CheckEnergyCrystal()
{
	while (true)
	{
	...
	for (WORD i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		...
		{
			if (invItem->GetSocket(1) > time(0) && slotIdx == -1)
				slotIdx = i;
			else
			{
	...
	if (slotIdx != -1)
	{
		LPITEM invItem = GetInventoryItem(slotIdx);
		if (invItem)
		{
			int lastTime = invItem->GetSocket(1) - time(0);
			if (lastTime <= 0)
			{
				invItem->SetSocket(0, 0);
				invItem->SetSocket(1, (invItem->GetSocket(1) - now) > 0 ? invItem->GetSocket(1) - now : 0);
			...
			for (BYTE i = 0; i < 5; ++i)
			{
				if (invItem->GetAttributeType(i) != APPLY_NONE)
					AddAffect(AFFECT_CRYSTAL, aApplyInfo[invItem->GetAttributeType(i)].bPointType, invItem->GetAttributeValue(i), AFF_NONE, lastTime, 0, true, true);
			}
		}
	}
}
