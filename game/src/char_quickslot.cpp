...
// QUICKSLOT HANDLING
/////////////////////////////////////////////////////////////////////////////
void CHARACTER::SyncQuickslot(DWORD bType, DWORD bOldPos, DWORD bNewPos)
{
	if (!m_PlayerSlots)
		return;
...
bool CHARACTER::GetQuickslot(DWORD pos, TQuickslot ** ppSlot)
{
	if (!m_PlayerSlots)
		return false;
...
bool CHARACTER::SetQuickslot(DWORD pos, TQuickslot & rSlot)
{
	if (!m_PlayerSlots)
		return false;
	...
	TItemPos srcCell(INVENTORY, (WORD)rSlot.pos);

	switch (rSlot.type)
	{
		case QUICKSLOT_TYPE_ITEM:
			if (srcCell.IsCostumeInventoryPosition())
				return false;
	...
	m_PlayerSlots->pQuickslot[(int)pos] = rSlot;

	if (GetDesc())
	{
		pack_quickslot_add.header	= HEADER_GC_QUICKSLOT_ADD;
		pack_quickslot_add.pos		= pos;
		pack_quickslot_add.slot		= m_PlayerSlots->pQuickslot[(int)pos];
...
bool CHARACTER::DelQuickslot(DWORD pos)
{
	if (!m_PlayerSlots)
		return false;
...
bool CHARACTER::SwapQuickslot(DWORD a, DWORD b)
{
	if (!m_PlayerSlots)
		return false;
...
void CHARACTER::ChainQuickslotItem(LPITEM pItem, DWORD bType, DWORD bOldPos)
{
	if (!m_PlayerSlots)
		return;
