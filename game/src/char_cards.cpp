...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == i)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot draw any more cards when you have already selected some."));
		return;
	}

	if (character_cards.cards_left < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You cannot draw any more cards."));
		return;
	}
...
#endif
	else if (character_cards.points > 0 && character_cards.points < 300)
#if defined(__WJ_PICKUP_ITEM_EFFECT__)
