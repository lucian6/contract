...
class CDungeon
{
	...
	bool	GetUniquePresence(const std::string& key);
	DWORD	GetUniqueMinHP(const std::string& key);
	DWORD	GetUniqueMaxHP(const std::string& key);
	void	AttackUniqueCharacter(long lMapIndex, const std::string& key);
	...
	void	UniqueSetAffect(const std::string& key, int type, int flag, int duration);
	void	UniqueRemoveAffect(const std::string& key, DWORD type);
	...
	void	UniqueSetImmortality(const std::string& key, bool bImmortality);
#endif
