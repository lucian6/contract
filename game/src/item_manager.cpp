...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	else
	{
		item->SetID(GetNewID());
#ifdef __SASH_SKIN__
		item->GiveDreamSoulBonus();
#endif
#ifdef __RENEWAL_CRYSTAL__
		item->SetCrystalBonus();
#endif
#ifdef __SASH_SKIN__
		item->ChangeDreamSoulBonus();
#endif
...
void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason)
{
	...
	if ((o = item->GetOwner()))
	{
		LogManager::instance().ItemLog(item->GetOwner(), item, "DESTROY", c_pszReason);
...
bool ITEM_MANAGER::GetDropPct(LPCHARACTER pkChr, LPCHARACTER pkKiller, OUT int& iDeltaPercent, OUT int& iRandRange)
{
	...
	int extraDrop = extraDrop += CPrivManager::instance().GetPriv(pkKiller, PRIV_ITEM_DROP) + ((pkKiller->IsEquipUniqueItem(70043) || pkKiller->IsEquipUniqueItem(72004) || pkKiller->IsEquipUniqueItem(72006)) ? 100 : 0);
	
	extraDrop += NEW_ITEM_DROP_RATE;
...
{
#ifdef __RENEWAL_CRYSTAL__
	if (pkOldItem->IsCrystal())
	{
		pkNewItem->SetSocket(1, pkOldItem->GetSocket(1));
		return;
	}
#endif
	// ACCESSORY_REFINE
