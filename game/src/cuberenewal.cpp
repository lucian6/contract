...
void Cube_open(LPCHARACTER ch)
{
	...
	if (ch->PreventTradeWindow(WND_CUBE, true))
	{
	...
	for (TCubeResultList::const_iterator iter = resultList.begin(); resultList.end() != iter; ++iter)
	{
		if (index_value == index)
		{
			...
			if (materialInfo.gold != 0) {
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You doesn't have the necessary amount of yang."));
					return;
				}
			}
			...
			if (materialInfo.gem != 0) {
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You doesn't have the necessary amount of gem."));
					return;
				}
			}
			...
			if (material_check) {
				...
				else if (pItem->IsSwitch())
					iEmptyPos = ch->GetEmptySwitchInventory(pItem->GetSize());
				else
				...
				WORD objectSlot = -1;
#ifdef ENABLE_CUBE_ATTR_SOCKET
				for (int i = 0; i < INVENTORY_MAX_NUM + SPECIAL_INVENTORY_MAX_PAGE_NUM; ++i)//Starting searchinf in inventory for Material Armor
				{
				...
				ch->SetStatistics(STAT_TYPE_CRAFT, 1);
				...
				{
#ifdef ENABLE_CUBE_ATTR_SOCKET
					if (materialInfo.allowCopyAttr == true && copyAttr != NULL && !pItem->IsDreamSoul() && !pItem->IsZodiacItem())
					{
						...
						if (pItem->GetType() == ITEM_WEAPON || (pItem->GetType() == ITEM_ARMOR && pItem->GetSubType() == ARMOR_BODY))
						{
				...
				if (pItem->IsZodiacItem())
					CHARACTER_MANAGER::Instance().SetRewardData(REWARD_FIRST_WEAPON_ZODIAC_CRAFT,ch->GetName(), true);
			}
			else
			{
