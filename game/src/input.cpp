...
bool CInputProcessor::Process(LPDESC lpDesc, const void * c_pvOrig, int iBytes, int & r_iBytesProceed)
{
	...
	for (m_iBufferLeft = iBytes; m_iBufferLeft > 0;)
	{
		...
		{
			sys_err("UNKNOWN HEADER: %d, LAST HEADER: %d(%d), REMAIN BYTES: %d, fd: %d, port: %d",
					bHeader, bLastHeader, iLastPacketLen, m_iBufferLeft, lpDesc->GetSocket(), lpDesc->GetPort());
...
BYTE IsHWIDBlocked(const char* c_szHWID)
{
	...
	char szQuery[QUERY_MAX_LEN + 1];
	snprintf(szQuery, sizeof(szQuery), "SELECT `ban` FROM `account`.`hwid` WHERE `hwid` = '%s' and `ban` = 1", szHWID);
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));
	...
	if (pRes->uiNumRows > 0)
	{
		...
		if (MYSQL_ROW row = mysql_fetch_row(pRes->pSQLResult))
		{
			str_to_number(bBanStatus, row[0]);

			if (bBanStatus > 0)
			{
				return HWID_STATUS_BLOCKED;
			}
		}
	}

	return HWID_STATUS_UNTOUCHED;
}
