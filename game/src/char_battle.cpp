...
void CHARACTER::RewardGold(LPCHARACTER pkAttacker)
{
	...
	if (CHARACTER_MANAGER::Instance().CheckTestDungeon(pkAttacker))
		return;
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		if (!vec_bSlots.empty())
		{
			...
			for (i = 0; i < iQty; ++i)
			{
				...
				if (IS_SET(pkItem->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_PKDROP) || IS_SET(pkItem->GetNormalFlag(), ITEM_FLAG_IRREMOVABLE) || pkItem->GetSocket(4) == 99)
					continue;
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller)
	{
		if (IsStone())
			...
			pkKiller->SetMissionBook(MISSION_BOOK_TYPE_MONSTER, 1, GetRaceNum(), GetLevel());
	}
#endif
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if (IsPC())
		{
			SetStatistics(STAT_TYPE_DEAD, 1);
	...
	if (pkKiller && pkKiller->IsPC())
	{
		if (IsPC())
		{
			switch (GetRealMapIndex())
			{
				case 110:
				case 111:
					pkKiller->SetStatistics(STAT_TYPE_GUILD_KILLS, 1);
			}
			if (GetEmpire() == 1)
				pkKiller->SetStatistics(STAT_TYPE_PC_A, 1);
			else if (GetEmpire() == 2)
				pkKiller->SetStatistics(STAT_TYPE_PC_B, 1);
			else if (GetEmpire() == 3)
				pkKiller->SetStatistics(STAT_TYPE_PC_C, 1);
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (GetRaceNum() == 20434)
	{
		const LPSECTREE_MAP c_lpSectreeMap = SECTREE_MANAGER::instance().GetMap(GetMapIndex());
		if (c_lpSectreeMap != nullptr)
		{
			auto victim = this;
			auto FBroadcastAllianceHP = [&victim](LPENTITY lpEntity)
			{
				...
				if (c_lpChar && c_lpChar->IsPC())
				{
					...
					Packet.dwVID = victim->GetVID();
					Packet.bAlliance = true;
					Packet.iAllianceMinHP = victim->GetHP();
					Packet.iAllianceMaxHP = victim->GetMaxHP();
	...
	if (pAttacker)
	{
		...
		if (pAttacker && pAttacker->IsPC() && GetRaceNum() == 6118)
		{
			if (CHARACTER_MANAGER::instance().CheckMeleyStatues(pAttacker))
			{
				int limitHP = CHARACTER_MANAGER::instance().CheckMeleyStatuesHP(pAttacker, GetMaxHP());
				dam = GetHP() - dam < limitHP ? GetHP() - limitHP : dam;
			}
		}

#ifdef __DUNGEON_INFO__
		if (pAttacker && pAttacker->IsPC() && !pAttacker->IsGM() && !IsPC() && IsZodiacBoss() && !CHARACTER_MANAGER::instance().CheckTestDungeon(pAttacker))
		{
			if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
			{
				char szBuff[254];
				snprintf(szBuff, sizeof(szBuff), "dungeon.2872_damage");
		...
		if (pAttacker && pAttacker->IsPC())
		{
			...
			{
				if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
					pAttacker->SetStatistics(STAT_TYPE_DMG_HIT_PC, dam);
				else
					pAttacker->SetStatistics(STAT_TYPE_DMG_SKILL_PC, dam);
			}
			else if (IsStone() && pAttacker->GetLevel() <= (GetLevel() + 20))
			{
				if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
					pAttacker->SetStatistics(STAT_TYPE_DMG_STONES, dam);
			}
		}
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	if (CHARACTER_MANAGER::Instance().CheckTestDungeon(to))
		return;
	...
	if (to->GetPoint(POINT_EXP_DOUBLE_BONUS))
		if (number(1, 100) <= to->GetPoint(POINT_EXP_DOUBLE_BONUS))
			rateFactor += 30;
	if (to->IsEquipUniqueItem(72002))
		rateFactor += 50;
	if (to->IsEquipUniqueItem(72001))
		rateFactor += 50;
	if (to->IsEquipUniqueItem(72003))
		rateFactor += 50;
	...
	if (pet)
		pet->PointChange(POINT_PET_EXP, iExp);
#endif
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (ch->GetMobAttackRange() > 150)
			{
				...
				ch->AddAffect(AFFECT_BOW_DISTANCE, POINT_BOW_DISTANCE, iNewRange - ch->GetMobAttackRange() + 1, AFF_NONE, 3*60, 0, false);
			}
		}
	}
};
