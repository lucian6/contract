...
DWORD CHARACTER::GetMyHorseVnum() const
{
	...
	LPITEM pkSkinMount = GetWear(WEAR_COSTUME_MOUNT_SKIN);
	if (pkSkinMount)
		return pkSkinMount->GetValue(3);
#endif

#ifdef __RENEWAL_MOUNT__
	LPITEM pkCostumeMount = GetWear(WEAR_COSTUME_MOUNT);
	if (pkCostumeMount)
		return pkCostumeMount->GetValue(3);
#endif
