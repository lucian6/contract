			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			*regen = tmp;
			...
			if (regen->type == REGEN_TYPE_MOB)
			{
				...
				else if (p->m_table.bType == CHAR_TYPE_NPC || p->m_table.bType == CHAR_TYPE_WARP || p->m_table.bType == CHAR_TYPE_GOTO || (p->m_table.bRank >= MOB_RANK_BOSS && p->m_table.bType == CHAR_TYPE_MONSTER))
				{
			...
			else if (regen->type == REGEN_TYPE_GROUP)
			{
				...
				if (pkGroup)
				{
					...
					else if (p->m_table.bRank >= MOB_RANK_BOSS && p->m_table.bType == CHAR_TYPE_MONSTER)
					{
						SECTREE_MANAGER::instance().InsertNPCPosition(lMapIndex,
							p->m_table.bType,
							c_rdwMembers[0],
