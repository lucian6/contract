...
struct npc_info
{
	...
	long x, y;
	DWORD time;
	npc_info(BYTE bType, DWORD vnum, long x, long y, DWORD time) : bType(bType), vnum(vnum), x(x), y(y), time(time){}
};
#else
struct npc_info
{
...
#ifdef ENABLE_DUNGEON_BOSS_ICON_IN_MAP
		void		InsertNPCPosition(long lMapIndex, BYTE bType, DWORD vnum, long x, long y, DWORD time);
#else
