...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	pack.count	= static_cast<DWORD>(pkItem->GetCount());
...
LPITEM CSafebox::GetItem(WORD bCell)
{
	if (bCell >= 5 * m_iSize)
	{
...
bool CSafebox::MoveItem(WORD bCell, WORD bDestCell, DWORD count)
{
	...
	if ((item->IsCostumeMountItem() && item->IsEquipped()) || (item->IsSkinMount() && item->IsEquipped()))
		return false;
...

	{
				...
				item2->GetVnum() == item->GetVnum())
		{
			...
			if (count == 0)
				count = static_cast<DWORD>(item->GetCount());
