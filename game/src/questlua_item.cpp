...
namespace quest
{
	...
	ALUA(item_get_attr_type)
	{
		...
		if (!lua_isnumber(L, 1))
		{
			sys_err("apply is not a number");
			lua_pushnumber(L, 0);
			return 1;
		}

		int apply = (int) lua_tonumber(L, 1);

		if (apply < 0 || apply >= ITEM_APPLY_MAX_NUM)
		{
			sys_err("index(%d) is out of range (0..%d)", apply, ITEM_APPLY_MAX_NUM);
			lua_pushnumber(L, 0);
		}
		else
			lua_pushnumber(L, item->GetValue(apply));
	...
	ALUA(item_get_attr_value)
	{
		...
		if (apply < 0 || apply >= ITEM_APPLY_MAX_NUM)
		{
			...
			lua_pushnumber(L, item->FindApplyValueQuest(apply));
...
#define NS_ITEM_GETMODE0(x)	\
	...
	ALUA(item_unequip0)
	{
		...
		bool unequip = false;
		if (item)
			unequip = ch->UnequipItem(item);

		lua_pushboolean(L, unequip);
