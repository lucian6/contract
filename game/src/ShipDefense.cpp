...
EVENTFUNC(LaserEffectEvent)
{
	...
	if (it != mapEffectPosition.end())
	{
		...
		if (static_cast<UINT>(number(1,100)) <= uiSpawnProbability)
		{
...
EVENTFUNC(WaveEvent)
{
	...
	case ShipDefense::WAVE4:
	{
		...
		{
			if (byWaitCount <= 3 || (byWaitCount % 10 == 0 && byWaitCount < 20))
				c_lpSectreeMap->for_each(FWaveNotice);
		}
	}
	break;
...
EVENTFUNC(SpawnEvent)
{
	...
	case ShipDefense::WAVE2:
	{
		if (static_cast<UINT>(number(1,1000)) <= 10) // 1%
			pShipDefense->SpawnEgg();

		if (static_cast<UINT>(number(1,1000)) <= 30) // 3%
	...
	case ShipDefense::WAVE3:
	{
		...
		if (static_cast<UINT>(number(1,1000)) <= 40) // 4%
	...
	case ShipDefense::WAVE4:
	{
		...
		if (static_cast<UINT>(number(1,1000)) <= 50) // 5%
...
void CShipDefense::Notice(const LPCHARACTER c_lpChar, const bool c_bBigFont, const char* c_pszBuf, ...)
{
	...
	va_start(pszArgs, c_pszBuf);
	{
		...
		c_lpChar->GetDesc()->ChatPacket(c_bBigFont ? CHAT_TYPE_MISSION : CHAT_TYPE_NOTICE, "%s", szBuf);
	}
	va_end(pszArgs);
}
...
void CShipDefense::SetFastestTime(int time)
{
	const LPSECTREE_MAP c_lpSectreeMap = GetSectreeMap();
	if (c_lpSectreeMap == nullptr)
		return;

	auto FFastestTime = [&time](LPENTITY lpEntity)
	{
		if (lpEntity->IsType(ENTITY_CHARACTER) == false)
			return;

		const LPCHARACTER c_lpChar = dynamic_cast<LPCHARACTER>(lpEntity);
		if (c_lpChar && c_lpChar->IsPC() && !c_lpChar->IsGM())
		{
			const int lastTime = c_lpChar->GetQuestFlag("dungeon.3962_fastest");
			if (lastTime != 0 && lastTime < time)
				return;
			c_lpChar->SetQuestFlag("dungeon.3962_fastest", time);
		}
	};
	c_lpSectreeMap->for_each(FFastestTime);
}

void CShipDefense::IncreaseCompletedDungeon()
{
	const LPSECTREE_MAP c_lpSectreeMap = GetSectreeMap();
	if (c_lpSectreeMap == nullptr)
		return;

	auto FCompletedDungeons = [](LPENTITY lpEntity)
	{
		if (lpEntity->IsType(ENTITY_CHARACTER) == false)
			return;
		...
		if (c_lpChar && c_lpChar->IsPC() && !c_lpChar->IsGM())
		{
			int count = c_lpChar->GetQuestFlag("dungeon.3962_completed");
			c_lpChar->SetQuestFlag("dungeon.3962_completed", count+1);
			c_lpChar->SetStatistics(STAT_TYPE_DUNGEONS, 1);
		}
	};
	c_lpSectreeMap->for_each(FCompletedDungeons);
}

void CShipDefense::UpdateInfoTimer()
{
	const LPSECTREE_MAP c_lpSectreeMap = GetSectreeMap();
	if (c_lpSectreeMap == nullptr)
		return;

	auto FUpdateTimer = [](LPENTITY lpEntity)
	{
		if (lpEntity->IsType(ENTITY_CHARACTER) == false)
			return;
		...
		if (c_lpChar && c_lpChar->IsPC())
		{
			c_lpChar->SendDungeonCooldown(0);
		}
	};
	c_lpSectreeMap->for_each(FUpdateTimer);
}
	...
	auto FNotice = [this, &c_eType](LPENTITY lpEntity)
	{
		...
		if (c_lpChar && c_lpChar->IsPC())
		{
			...
			case NOTICE_WAVE1:
			{
				...
				c_lpChar->ChatPacket(CHAT_TYPE_COMMAND, "RefreshDungeonTimer %d %d", GetWave() + 1, ShipDefense::FIRST_WAVE_DURATION + 21);
			}
			break;
			...
			case NOTICE_WAVE4:
			{
				...
				c_lpChar->ChatPacket(CHAT_TYPE_COMMAND, "RefreshDungeonTimer %d %d", GetWave() + 1, 480);
			}
			break;
			...
			case NOTICE_WAVES:
				...
				c_lpChar->ChatPacket(CHAT_TYPE_COMMAND, "RefreshDungeonTimer %d %d", GetWave(), JUMP_OUT_DELAY);
				break;
...
bool CShipDefenseManager::Create(const LPCHARACTER c_lpChar)
{
	...
	if (pShipDefense != nullptr)
	{
		...
		if (c_lpParty != nullptr)
		{
			...
			{
				auto FSetCooldown = [&c_lpParty, pShipDefense](const LPCHARACTER c_lpChar)
				{
					if (c_lpChar != nullptr && c_lpChar->GetQuestFlag("ship_defense.test_dungeon") == 0)
					{
						auto event = CHARACTER_MANAGER::Instance().CheckEventIsActive(HALF_TIMER_DUNGEONS_EVENT, c_lpChar->GetEmpire());
						int iTime = event ? time(0) + ShipDefense::COOLDOWN_HALF : time(0) + ShipDefense::COOLDOWN;

						c_lpChar->SetQuestFlag("ship_defense.cooldown", iTime);
					}
		...
		{
			if (ShipDefense::REQUIRE_COOLDOWN == true && c_lpChar != nullptr && c_lpChar->GetQuestFlag("ship_defense.test_dungeon") == 0)
			{
				auto event = CHARACTER_MANAGER::Instance().CheckEventIsActive(HALF_TIMER_DUNGEONS_EVENT, c_lpChar->GetEmpire());
				int iTime = event ? time(0) + ShipDefense::COOLDOWN_HALF : time(0) + ShipDefense::COOLDOWN;
				c_lpChar->SetQuestFlag("ship_defense.cooldown", iTime);
		...
		if (c_lpParty != nullptr)
		{
			...
			c_lpParty->ForEachOnlineMember(FWarp);
		}
		else {
			// NOTE : Warp to Private Map
			PIXEL_POSITION SPos = { 0, 0, 0 };
...
bool CShipDefenseManager::StartTest(const LPCHARACTER c_lpChar)
{
	if (c_lpChar == nullptr)
		return false;
...
void CShipDefenseManager::SetFastest(const LPCHARACTER c_lpChar, int time)
{
	if (c_lpChar == nullptr)
		return;

	const DWORD c_dwLeaderPID = GetLeaderPID(c_lpChar, true);
	...
	if (it != m_mapShipDefense.end())
	{
		CShipDefense* pShipDefense = it->second;
		if (pShipDefense != nullptr)
			pShipDefense->SetFastestTime(time);
	}
}

void CShipDefenseManager::IncreaseCompleted(const LPCHARACTER c_lpChar)
{
	if (c_lpChar == nullptr)
		return;
	...
	if (it != m_mapShipDefense.end())
	{
		CShipDefense* pShipDefense = it->second;
		if (pShipDefense != nullptr)
			pShipDefense->IncreaseCompletedDungeon();
	}
}


void CShipDefenseManager::UpdateInfo(const LPCHARACTER c_lpChar)
{
	if (c_lpChar == nullptr)
		return;
...
bool CShipDefenseManager::CanJoin(const LPCHARACTER c_lpChar)
{
	...
	ShipDefenseMap::const_iterator f = m_mapShipDefense.find(c_dwLeaderPID);
...
bool CShipDefenseManager::SetTest(const LPCHARACTER c_lpChar, bool state)
{
	if (c_lpChar == nullptr)
		return false;
	...
	if (it != m_mapShipDefense.end())
	{
		CShipDefense* pShipDefense = it->second;
		if (pShipDefense != nullptr)
			pShipDefense->SetTest(state);
		return true;
	}
	return false;
}

bool CShipDefenseManager::GetTest(const LPCHARACTER c_lpChar)
{
	if (c_lpChar == nullptr)
		return false;
	...
	if (lpKillerChar != nullptr)
	{
		...
		if (it != m_mapShipDefense.end())
		{
			...
			if (pShipDefense)
			{
				...
				if (lpDeadChar->GetRaceNum() == HYDRA3)
					CHARACTER_MANAGER::Instance().DoMission(lpKillerChar, MISSION_DUNGEON, 1, lpDeadChar->GetRaceNum());
